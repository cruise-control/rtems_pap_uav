...
#if defined USE_AHRS && defined USE_IMU
...
static inline uint8_t pprz_mode_update(void) {
	...
	|| TRUE
		...
		return ModeUpdate(pprz_mode,
				PPRZ_MODE_OF_PULSE(fbw_state->channels[RADIO_MODE]));
#else
...
static inline void telecommand_task(void) {
	...
	uint8_t really_lost =
	bit_is_set(fbw_state->status, STATUS_RADIO_REALLY_LOST)
	...
	if (pprz_mode == PPRZ_MODE_AUTO1) {
		...
		h_ctl_pitch_setpoint = FLOAT_OF_PPRZ(fbw_state->channels[RADIO_PITCH],
				0., AUTO1_MAX_PITCH);
	...
	if (!autopilot_flight_time) {
		if (pprz_mode == PPRZ_MODE_AUTO2
				&& fbw_state->channels[RADIO_THROTTLE]
