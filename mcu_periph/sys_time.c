...
static char timer_name[SYS_TIME_NB_TIMER][4] = { { 'T', 'I', 'M', '0' }, { 'T',
		'I', 'M', '1' }, { 'T', 'I', 'M', '2' }, { 'T', 'I', 'M', '3' }, { 'T',
		'I', 'M', '4' }, { 'T', 'I', 'M', '5' }, { 'T', 'I', 'M', '6' }, { 'T',
		'I', 'M', '7' }, { 'T', 'I', 'M', '8' }, { 'T', 'I', 'M', '9' } };
...
void sys_time_init(void) {
	...
	//sys_time.ticks_per_sec = rtems_clock_get_ticks_per_second();
	sys_time.ticks_per_sec = rtems_clock_get_ticks_per_second(); //As there are 1000 microseconds per tick, each tick is a millisecond
...
int sys_time_register_timer(float duration, sys_time_cb cb) {
	...
	if (empty_slot == SYS_TIME_NB_TIMER) { //Timers occupancy are full
		UART1Puts("No spaces left!\r\n");
		return -1;
	}

	status = rtems_timer_create(
			rtems_build_name(timer_name[empty_slot][0],
					timer_name[empty_slot][1], timer_name[empty_slot][2],
					timer_name[empty_slot][3]), &timerID);

	if (status) {
		UART1Puts("Count not create a timer!\r\n");
		return -1;
	}

	//initialize the particular timer
	sys_time.timer[empty_slot].cb = cb;
	sys_time.timer[empty_slot].duration = (duration
			* (sys_time.ticks_per_sec * 1.))/*ticks_per_second*/;
	char buf[200];
	buf[0] = '\0';
	sprintf(buf, "duration %f :: %ld\r\n",
			(duration * (sys_time.ticks_per_sec * 1.)),
	...
	status = rtems_timer_fire_after(sys_time.timer[empty_slot].sys_id,
			sys_time.timer[empty_slot].duration, timer_callback,
			(void *) (&sys_time.timer[empty_slot]));
	if (status) {
		//initialize the particular timer
...
void sys_time_update_timer(tid_t id, float duration) {
	...
	sys_time.timer[id].duration =
			(duration * sys_time.ticks_per_sec * 1.0) /*ticks_per_second*/;
	rtems_timer_fire_after(sys_time.timer[id].sys_id,
			sys_time.timer[id].duration, timer_callback,
			(void *) (&sys_time.timer[id]));
	return;
}
